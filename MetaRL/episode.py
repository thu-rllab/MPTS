...
class BatchEpisodes(object):
    def __init__(self, batch_size, gamma=0.95, device='cpu'):
        ...
        self._mask_list = [[] for _ in range(batch_size)]
    ...
    def mask(self):
        ...
        #     for i in range(self.batch_size):
        #         length = len(self._mask_list[i])
        #         mask[:length, i] = np.stack(self._mask_list[i], axis=0)
        #     self._mask = torch.from_numpy(mask).to(self.device)
        # return self._mask
    ...
    def append(self, observations, actions, rewards, batch_ids, infos, dones):
        for observation, action, reward, batch_id, info, done in zip(
                observations, actions, rewards, batch_ids, infos, dones):
            if batch_id is None:
                continue
            ...
            self._infos_list[batch_id].append(info)
            self._mask_list[batch_id].append(1.0 - done)
